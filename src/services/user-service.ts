...
async function getDashboardData(user_id: number) {
    try {
        ...
        const lastRegisterQuery = await database.$queryRaw`
            select mg_per_dl, date_format(created_at,'%d/%m/%Y %H:%i:%S') as created_at from glucose_record where user_id = ${user_id} order by glucose_record.created_at desc limit 1;
        `;
